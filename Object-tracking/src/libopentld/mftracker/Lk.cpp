...
const int MAX_COUNT = 500; //!< It isn't used
const int MAX_IMG = 2; //!< the maximum number of image used for track points
const double N_A_N = -1.0; //!< the value whith replaces NaN
int win_size_lk = 4; //!< the size of the search window of each pyramid level in cvCalcOpticalFlowPyrLK
CvPoint2D32f *points[3] = { 0, 0, 0 }; //!< the array of source points in Image 1; the array of tracked points in Image 2; the array of forward-backward points in Image 1
static IplImage **PYR = 0; //!< the array of the pyramid for every frame.

/**
 * @brief Calculates euclidean distance between the point pairs.
...
/**
 * @brief Calculates normalized cross correlation for every point.
 * @param imgI      Image 1.
 * @param imgJ      Image 2.
