...
void tldNormalizeImg(const cv::Mat &img, float *output)
{
	...
	result = cv::Mat(&Result); // cv::Mat::Mat(const CvMat* m, bool copyData = false);
	// or
	result = cv::Mat(&Result, true); // t
	*output = cvNorm(&Result, &normalizeRes, CV_L2);

}

/**
 * @brief Creates new rectangle from boundary.
 *
 * @param boundary
 *          the array (size 4)
...
/**
 * @brief Calculates the share overlap of two bounding boxes (the share the intersection area in the occupy area).
 *
 * @param bb1
 *          the one bounding box
 * @param bb2
 *          the two bounding box
