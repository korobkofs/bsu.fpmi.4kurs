...
void Main::doWork()
{
    ...
    if(selectManually)
    {
        ...
        box = Main::maxMovingRegion(grey);
        if(box.width == 0)
                {
                    printf("End of file or cannot grab frame \"%s\"\n", printResults);
                    return;
                }
    ...
    while(imAcqHasMoreFrames(imAcq))
    {
        ...
        if(showOutput || saveDir != NULL)
        {
            ...
            sprintf(string, "#%d,Posterior %.2f; fps: %.2f, #numwindows:%d, %s, %d:numobject", imAcq->currentFrame - 1, tld->currConf, fps, tld->detectorCascade->numWindows, learningString, numobject);
            ...
            if(tld->currBB != NULL)
            {
                ...
                cvRectangle(img, tld->currBB->tl(), tld->currBB->br(), rectangleColor, 1/*8*/, 8, 0);
        ...
        if(!reuseFrameOnce)
        {
            if (tld->currBB == NULL) {
                ...
                if (Main::isMotionInBox(tld->prevImg(*(tld->currBB)), tld->currImg(*(tld->currBB)))) {
                    count_stop = 0;
                } else {
                    count_stop++;
                }
