...
/**
 * @brief In this class all settings are stored.
 */
class Settings
{
public:
    ...
    /**
     * @brief This constructor sets the settings by default.
     *
     */
    Settings();
     ...
     */
    ~Settings();
    bool m_trackerEnabled; //!< enables tracker (Lucas-Kanade)
    bool m_varianceFilterEnabled; //!< enables  variance filter
    bool m_ensembleClassifierEnabled; //!< enables ensemble classifier
    bool m_nnClassifierEnabled; //!< enables nearest neighbor сlassifier
    ...
    float m_thetaP; //!< upper threshold for confidence in lerning and filter of NNClassifier
    float m_thetaN; //!< lower threshold for confidence in filter of NNClassifier
    int m_seed; //!< the seed for pseudo-random number generator
    ...
    std::string m_printResults; //!< path to the file where the results should be printed; NULL -> results will not be printed
